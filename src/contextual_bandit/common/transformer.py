...
class Model(nn.Module):
    def __init__(
        ...
        self.transformer = nn.ModuleDict(
            dict(
                ...
                out_proj=nn.Linear(config.n_embd, token_dim),
                reward_pred_head=nn.Linear(config.n_embd, 1),
            )
        )
    ...
    def forward(
        ...
        action_set_prompt: torch.Tensor,
        max_seq_length: Optional[int] = None,
    ) -> torch.Tensor:
        b, t = rewards.shape[:2]
        T = rewards.shape[1] * 3 + action_set_prompt.shape[1]
        ...
        # Forward
        rew_emb = self.transformer.reward_emb(rewards[..., None])
        ...
        if self.use_action_set_prompt:
            sequence = torch.cat([action_set_prompt, sequence], dim=1)
            acts_pool_len = action_set_prompt.shape[1]
        else:
            acts_pool_len = 0
